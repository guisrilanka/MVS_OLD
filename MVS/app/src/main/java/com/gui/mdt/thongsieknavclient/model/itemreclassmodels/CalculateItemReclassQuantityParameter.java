package com.gui.mdt.thongsieknavclient.model.itemreclassmodels;

/**
 * Created by user on 12/29/2016.
 */

public class CalculateItemReclassQuantityParameter {

    private String UserCompany;
    private String UserName;
    private String Password;
    private float Quantity;
    private float QuantityPerUom;

    public CalculateItemReclassQuantityParameter(String userCompany, String userName, String password, float quantity, float quantityPerUom) {
        UserCompany = userCompany;
        UserName = userName;
        Password = password;
        Quantity = quantity;
        QuantityPerUom = quantityPerUom;
    }

    public String getUserCompany() {
        return UserCompany;
    }

    public void setUserCompany(String userCompany) {
        UserCompany = userCompany;
    }

    public String getUserName() {
        return UserName;
    }

    public void setUserName(String userName) {
        UserName = userName;
    }

    public String getPassword() {
        return Password;
    }

    public void setPassword(String password) {
        Password = password;
    }

    public float getQuantity() {
        return Quantity;
    }

    public void setQuantity(float quantity) {
        Quantity = quantity;
    }

    public float getQuantityPerUom() {
        return QuantityPerUom;
    }

    public void setQuantityPerUom(float quantityPerUom) {
        QuantityPerUom = quantityPerUom;
    }
}
