...
//                    @Override
//                    public void run() {
//                        Toast.makeText(mApp.getBaseContext(), "Start print background taskn", Toast.LENGTH_LONG).show();
//                    }
//                });
                mLog.info("Start print background task");
                printDraft();
                mLog.info("end print background task");
//                mainHandler.post(new Runnable() {
//                    @Override
//                    public void run() {
//                        Toast.makeText(mApp.getBaseContext(), "end print background task", Toast.LENGTH_LONG).show();
//                    }
//                });
            } catch (Exception e) {
                mainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(mApp.getBaseContext(), e.getMessage(), Toast.LENGTH_LONG).show();
                    }
                });

                mLog.info("Print doinbackgound get issue" + e.toString());
...
//                    docDPL.writeImage(anImage, row, 120, paramDPL);
            ...
            mLog.info("Print pdf  get issue" + ex.toString());
            mainHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(mApp.getBaseContext(), ex.getMessage(), Toast.LENGTH_LONG).show();
                }
            });
        }
    ...
    public void run() {
        ...
        try {
            ...
            mLog.info("Print run  get issue" + e.toString());
            //signals to close connection
            if (conn != null)
                conn.close();
            e.printStackTrace();
